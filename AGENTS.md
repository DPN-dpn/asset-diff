...
### 2. 모드 Diff 매칭 (`ModDiffPage`)

자동 매칭의 불안정성을 해결하기 위해, 사용자가 직접 구버전 모드 INI 섹션과 신규 에셋 해시를 1:1로 수동 짝지어 `diff.json`을 내보냅니다.

**동작 특징:**
1. **듀얼 캔버스 뷰**: 좌측은 텍스트 내용(섹션/힌트)을 스크롤하고 우측은 콤보박스를 고정시켜 가독성과 조작성을 극대화합니다.
2. **이중 콤보박스**: 먼저 에셋 파츠(예: Face, Body)를 선택하면, 두 번째 콤보박스에 해당 파츠가 가진 실제 해시 목록(`draw_vb`, `Diffuse` 등)이 동적으로 나타납니다.
3. **실시간 검색 필터링**: 섹션 이름과 힌트 내용을 대소문자 구분 없이 검색하여 목록을 좁힐 수 있습니다.
4. **추론 배제**: 모호한 휴리스틱 로직을 제거하고 오직 사용자가 명시적으로 선택한 매핑 쌍만 Diff로 추출합니다.
...
│       ├── ModDiffPage  (구버전 모드 INI 수동 매칭 및 듀얼 캔버스 뷰)
│       └── ScriptPage   (픽스툴 작성)
